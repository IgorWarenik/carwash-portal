...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    citySellPages = cities.map(c => ({
      url: `${BASE_URL}/prodaty-avtomoiku/${c.slug}`,
      lastModified: new Date(),
      changeFrequency: 'weekly' as const,
      priority: 0.9,
    }))
    ...
    cityBeskontaktPages = cities.map(c => ({
      url: `${BASE_URL}/avtomoyki/${c.slug}/beskontaktnaya`,
      lastModified: new Date(),
      changeFrequency: 'weekly' as const,
      priority: 0.8,
    }))
    cityHimchistkaPages = cities.map(c => ({
      url: `${BASE_URL}/avtomoyki/${c.slug}/himchistka-salona`,
      lastModified: new Date(),
      changeFrequency: 'weekly' as const,
      priority: 0.8,
    }))
    cityReviewPages = cities.map(c => ({
      url: `${BASE_URL}/avtomoyki/${c.slug}/otzyvy`,
      lastModified: new Date(),
      changeFrequency: 'daily' as const,
      priority: 0.8,
    }))
    ...
    cityBudgetPages = cities.map(c => ({
      url: `${BASE_URL}/avtomoyki/${c.slug}/deshevo`,
      lastModified: new Date(),
      changeFrequency: 'weekly' as const,
      priority: 0.8,
    }))
    const TYPE24H_SLUGS = ['samobsluzhivanie', 'ruchnaya', 'avtomaticheskaya', 'deteyling', 'gruzovye']
    cityType24hPages = cities.flatMap(c =>
      TYPE24H_SLUGS.map(t => ({
        url: `${BASE_URL}/avtomoyki/${c.slug}/${t}-kruglosutochno`,
        lastModified: new Date(),
        changeFrequency: 'weekly' as const,
        priority: 0.8,
      }))
  ...
  return [...staticPages, ...blogPages, ...toolPages, ...franchisePages, ...cityPages, ...cityTypePages, ...cityBuyPages, ...citySellPages, ...cityOpenPages, ...cityPricePages, ...cityBeskontaktPages, ...cityHimchistkaPages, ...cityReviewPages, ...cityRatingPages, ...cityBudgetPages, ...city24hPages, ...cityType24hPages, ...districtPages, ...carwashPages]
}
