...
// Type-SEO slugs (merged here to avoid Next.js parallel dynamic segment conflict)
...
// Type+24h slugs (formerly [typeslug]-kruglosutochno folder — merged to avoid parallel dynamic segment conflict)
const TYPE_24H_MAP: Record<string, { db: string; label: string; labelRod: string; labelVin: string }> = {
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  // Type+24h page
  const type24hInfo = TYPE_24H_MAP[params.slug]
  if (type24hInfo) {
    try {
      const city = await prisma.city.findUnique({ where: { slug: params.city } })
      if (!city) return {}
      return {
        title: `Мойка ${type24hInfo.labelVin} круглосуточно в ${city.name} — 24/7`,
        description: `Круглосуточные мойки ${type24hInfo.labelRod} в ${city.name}: работают 24 часа, 7 дней в неделю. Адреса, телефоны, цены.`,
...
export default async function CarWashDetailPage({ params }: Props) {
  // Type+24h page (formerly [typeslug]-kruglosutochno/ folder — merged to avoid parallel dynamic segment conflict)
  const type24hInfo = TYPE_24H_MAP[params.slug]
  if (type24hInfo) {
    ...
    const carwashes = await prisma.carWash.findMany({
      where: { cityId: city.id, status: 'active', type: type24hInfo.db as never, isOpen24h: true },
    ...
    const breadcrumbLd = {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 4, name: `${type24hInfo.label} 24/7`, item: `https://www.businessmoyka.ru/avtomoyki/${params.city}/${params.slug}` },
      ],
    }
    const faqLd = {
      '@context': 'https://schema.org',
      '@type': 'FAQPage',
      mainEntity: [
        { '@type': 'Question', name: `Есть ли мойки ${type24hInfo.labelRod} в ${city.name}, которые работают ночью?`, acceptedAnswer: { '@type': 'Answer', text: `Да, в ${city.name} работает ${carwashes.length} круглосуточных мойки ${type24hInfo.labelRod}. Все принимают клиентов 24/7, включая ночь и праздники.` } },
        { '@type': 'Question', name: `Сколько стоит ${type24hInfo.labelVin} ночью в ${city.name}?`, acceptedAnswer: { '@type': 'Answer', text: `Цены ночью и днём одинаковые. ${type24hInfo.db === 'self_service' ? 'Мойки самообслуживания стоят от 80 до 350 ₽ за сеанс.' : 'Уточняйте цены в карточке каждой мойки.'}` } },
      ],
    }
    const itemListLd = {
      '@context': 'https://schema.org', '@type': 'ItemList',
      name: `${type24hInfo.label} круглосуточно в ${city.name}`, numberOfItems: carwashes.length,
      itemListElement: carwashes.map((cw, i) => ({ '@type': 'ListItem', position: i + 1, item: { '@type': 'LocalBusiness', name: cw.name, address: { '@type': 'PostalAddress', streetAddress: cw.address, addressLocality: city.name }, openingHours: 'Mo-Su 00:00-24:00' } })),
    }
    ...
    return (
      <main className="max-w-5xl mx-auto px-4 py-12">
        ...
        <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2 flex-wrap">
          <Link href="/" className="hover:text-[#e94560]">Главная</Link><span>›</span>
          <Link href="/avtomoyki" className="hover:text-[#e94560]">Автомойки</Link><span>›</span>
          <Link href={`/avtomoyki/${params.city}`} className="hover:text-[#e94560]">{city.name}</Link><span>›</span>
          <span className="text-gray-900">{type24hInfo.label} 24/7</span>
        </nav>
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="text-xs bg-gray-100 text-gray-600 px-3 py-1.5 rounded-full">{type24hInfo.label}</span>
        </div>
        <h1 className="text-3xl font-bold mb-2">Мойка {type24hInfo.labelVin} круглосуточно в {city.name}</h1>
        <p className="text-gray-500 mb-8">{carwashes.length} {carwashes.length === 1 ? 'мойка' : 'моек'} — работают 24 часа, 7 дней в неделю</p>
          ...
          {carwashes.map(cw => (
            <Link key={cw.id} href={`/avtomoyki/${params.city}/${cw.slug}`} className="bg-white border border-gray-200 rounded-2xl p-5 hover:border-[#e94560] hover:shadow-md transition-all flex flex-col group">
              <div className="flex items-start justify-between mb-3">
                <span className="text-xs font-medium bg-gray-100 text-gray-600 px-2 py-1 rounded-full">{type24hInfo.label}</span>
              ...
              <div className="mt-auto space-y-1.5 text-sm">
                {cw.rating !== null && cw.rating > 0 && (
                  <div className="flex items-center gap-1 text-yellow-500">
                    <span>★</span><span className="font-medium text-gray-900">{cw.rating.toFixed(1)}</span>
                    <span className="text-gray-400">({cw.reviewCount})</span>
                  </div>
                )}
                {cw.priceFrom && <p className="text-gray-600">от <span className="font-semibold">{cw.priceFrom} ₽</span></p>}
        ...
        <section className="mb-12">
          ...
          <div className="space-y-3">
            {faqLd.mainEntity.map(item => (
              <details key={item.name} className="group bg-white border border-gray-200 rounded-xl overflow-hidden">
                ...
                <p className="px-5 pb-4 text-sm text-gray-600 leading-relaxed">{item.acceptedAnswer.text}</p>
              </details>
            ))}
          </div>
        </section>
        <div className="flex flex-wrap gap-4">
          <Link href={`/avtomoyki/${params.city}/kruglosutochno`} className="px-6 py-3 bg-[#e94560] text-white rounded-xl font-semibold hover:bg-[#c73652] transition-colors text-sm">Все 24/7 в {city.name}</Link>
          <Link href={`/avtomoyki/${params.city}/${params.slug.replace('-kruglosutochno', '')}`} className="px-6 py-3 border border-gray-200 text-gray-700 rounded-xl font-semibold hover:bg-gray-50 transition-colors text-sm">Все {type24hInfo.labelRod} →</Link>
          <Link href={`/avtomoyki/${params.city}`} className="px-6 py-3 border border-gray-200 text-gray-700 rounded-xl font-semibold hover:bg-gray-50 transition-colors text-sm">Все мойки →</Link>
        </div>
      </main>
    )
  }
